...
class IngredientSpellChecker:
    ...
    def parse_ingredient_list(self, text: str) -> List[str]:
        ...
        # Replace newlines with spaces (ingredients can wrap across lines)
        text = text.replace('\n', ' ')
        ...
        for phrase in stop_phrases:
            ...
            if match:
                # Get everything before the stop phrase
                text_before = text[:match.start()]
                
                # Strip any trailing non-ingredient characters (like +, *, newlines, spaces)
                text_before_stripped = text_before.rstrip('+*#@!\n ')
                
                # Now use the stripped version
                text = text_before_stripped
                break
    ...
    def correct_ocr_text(self, ocr_text: str, threshold: float = 0.70) -> Dict:
        ...
        print("\n" + "=" * 80)
        print("DEBUG - PARSED INGREDIENTS (before correction):")
        print("=" * 80)
        for i, ing in enumerate(ingredients, 1):
            print(f"{i:2}. {repr(ing)}")
        print("=" * 80)
