...
class IngredientSpellChecker:
    ...
    def find_best_match(self, ingredient: str, threshold: float = 0.70) -> Tuple[str, float]:
        ...
        # Only do this for single or two-word ingredients
        word_count = len(ingredient_clean.split())
        if word_count <= 2:
            # Look for ingredients that START with this text
            for db_ing_lower, db_ing in zip(self.db.ingredients_lower, self.db.ingredients):
                if db_ing_lower.startswith(ingredient_clean.lower() + ' '):
                    # Found a longer version - this is likely the correct one
                    return db_ing, 0.95  # High confidence
