...
class ComprehensiveIngredientDatabase:
    ...
    def __init__(self):
        ...
        self.ingredients = [
            ...
            "Creatine", "Ubiquinone", "Coenzyme Q10",
            ...
            # UV Filters
            "Octocrylene", "Butyl Methoxydibenzoylmethane", "Avobenzone",
...
class IngredientSpellChecker:
    ...
    def find_best_match(self, ingredient: str, threshold: float = 0.70) -> Tuple[str, float]:
        ...
        # Check if this might be an incomplete ingredient name
        # e.g., "Stearyl" should match "Stearyl Alcohol"
        # "Butyrospermum Parkii" should match "Butyrospermum Parkii Butter"
        # But NOT generic words like "Alcohol" alone
        words = ingredient_clean.split()
        word_count = len(words)
        
        # Don't expand these generic words
        exclude_words = ['alcohol', 'extract', 'oil', 'acid', 'butter']
        
        if word_count == 1:
            # Single word - only if not in exclude list
            if ingredient_clean.lower() not in exclude_words and len(ingredient_clean) >= 6:
                for db_ing_lower, db_ing in zip(self.db.ingredients_lower, self.db.ingredients):
                    if db_ing_lower.startswith(ingredient_clean.lower() + ' '):
                        return db_ing, 0.95
        elif word_count == 2 and all(len(w) >= 4 for w in words):
            # Two words, each at least 4 chars (e.g., "Butyrospermum Parkii")
            for db_ing_lower, db_ing in zip(self.db.ingredients_lower, self.db.ingredients):
                if db_ing_lower.startswith(ingredient_clean.lower() + ' '):
                    return db_ing, 0.95
    ...
    def parse_ingredient_list(self, text: str) -> List[str]:
        ...
        # Use fuzzy matching to handle OCR errors like "PIGREDIENTS", "TNGREDIENTS"
        # Look for pattern: (any 1-2 chars) + "NGREDIENTS" or just "INGREDIENTS"
        match = re.search(r'\w{0,2}ngredients?\s*:?\s*', text, flags=re.IGNORECASE)
        if match:
            text = text[match.end():]
        
        # IMPORTANT: Handle hyphenated line breaks (word continues on next line)
        # "B-\nutylene" -> "Butylene", "Polyac-\nrylate" -> "Polyacrylate"
        text = re.sub(r'-\s*\n\s*', '', text)
        
        # Replace remaining newlines with spaces (ingredients can wrap across lines)
        text = text.replace('\n', ' ')
        
        # Protect commas in chemical names (e.g., "1,2-Hexanediol", "1,3-Butanediol")
        # Replace digit,digit with digit|digit temporarily
        ...
        stop_phrases = [
            ...
            r'\bm\.?r\.?p\b',
            r'\bno\s+animal\b',
            r'\banimal\s+derived\b',
            r'\bfri\s+fr[aå]n\b',  # Swedish: "free from"
            r'\buden\s+\w+\b',  # Swedish: "without"
            r'\b\+no\s',  # Random packaging markers
        ]
        ...
        # Split by common delimiters (comma, semicolon, and period at end of ingredient)
        # Period followed by space and capital letter indicates new ingredient
        raw_ingredients = re.split(r'[,;]\s*|\.\s+(?=[A-Z])', text)
        ...
        for ing in raw_ingredients:
            ...
            # Restore commas in chemical names (we replaced digit,digit with digit|digit)
            ing = ing.replace('|', ',')
            ...
            # Work on individual words to catch them even with numbers in between
            ocr_word_fixes = {
                'haltooligosy': 'Maltooligosyl',
                'maltooligosyl': 'Maltooligosyl',
                'vatolsate': 'Hydrolysate',
                'hydrolsate': 'Hydrolysate',
                'nttylsothiazolinone': 'Methylisothiazolinone',
                'ttylisothiazolinone': 'Methylisothiazolinone',
                ...
                'buty!': 'butyl',  # ! → l
                'hexy!': 'hexyl',  # ! → l
                'isomethy!': 'isomethyl',  # ! → l
            }
            
            # Split into words, fix each word, then rejoin
            words = ing.split()
            fixed_words = []
            for word in words:
                ...
                if word_clean in ocr_word_fixes:
                    fixed_words.append(ocr_word_fixes[word_clean])
                else:
                    fixed_words.append(word)
            
            ing = ' '.join(fixed_words)
            
            # Skip if this looks like junk
            if len(ing) < 50 and any(word in ing.lower() for word in ['ingredient', 'label', 'sale']):
                continue
            
            # Skip standalone generic words that should never appear alone
            if ing.lower().strip() in ['alcohol', 'extract', 'oil', 'acid', 'butter', 'water', 'cetyl']:
                continue
            
            # Special handling for "&" separated ingredients
            # Keep them together if they look like a proper ingredient pair
            if '&' in ing and '(and)' not in ing.lower():
                ...
                if len(parts) == 2:
                    ...
                    # Keep together if:
                    # 1. Both parts are multi-word (e.g., "Maltooligosyl Glucoside & Hydrogenated Starch Hydrolysate")
                    # 2. OR both parts are long single words (e.g., "Methylchloroisothiazolinone & Methylisothiazolinone")
                    part1_words = len([w for w in part1.split() if len(w) > 2])
                    part2_words = len([w for w in part2.split() if len(w) > 2])
                    part1_long = len(part1.replace(' ', '')) > 15  # Long ingredient name
                    part2_long = len(part2.replace(' ', '')) > 15
                    
                    should_keep_together = (
                        (part1_words >= 2 and part2_words >= 2) or  # Both multi-word
                        (part1_long and part2_long)  # Both long single words
                    )
                    ...
                    if should_keep_together:
                        ...
                        # Split them into separate ingredients
                        for part in [part1, part2]:
                            part_clean = re.sub(r'[^a-zA-Z0-9\s\-]', ' ', part)
                            part_clean = ' '.join(part_clean.split())
                            if part_clean and len(part_clean) > 2:
                                normalized = part_clean.lower().strip()
                                if normalized not in seen:
            ...
            if '(and)' in ing.lower():
                ...
                if not was_split:
                    ...
                    # Remove special characters (but keep hyphens, ampersands, and commas)
    ...
    def correct_ocr_text(self, ocr_text: str, threshold: float = 0.70) -> Dict:
        ...
        for original in ingredients:
            # If this contains &, it's already been corrected during parsing
            # Don't fuzzy match it - keep it as-is
            if '&' in original:
                corrections.append({
                    'original': original,
                    'corrected': original,  # Keep as-is
                    'confidence': 1.0,
                    'changed': False
                })
                matched += 1
...
def main():
    """Main execution"""
    # Find images (both uppercase and lowercase extensions)
    ...
    for image_path in images:
        result = ocr_and_correct(str(image_path), threshold=0.70)
