...
class IngredientSpellChecker:
    ...
    def parse_ingredient_list(self, text: str) -> List[str]:
        ...
        for ing in raw_ingredients:
            ...
            # Handle "&" separated ingredients (e.g., "Ingredient1 & Ingredient2")
            # Split these into separate entries
            if '&' in ing and '(and)' not in ing.lower():
                parts = [p.strip() for p in ing.split('&')]
                for part in parts:
                    # Clean each part
                    part = re.sub(r'^\s*[a-z0-9]\s+', '', part, flags=re.IGNORECASE)
                    part = re.sub(r'\s+[a-z0-9]\s*$', '', part, flags=re.IGNORECASE)
            ...
            if '(and)' in ing.lower():
                ...
                # Try to split on common ingredient words
                split_keywords = ['fragrance', 'parfum', 'extract', 'oil', 'acid', 'alcohol']
                was_split = False
